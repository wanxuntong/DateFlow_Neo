    ...
    def get_task_data(self):
        ...
        try:
            ...
            task_data = {
                ...
                "urgency": urgency,
                "completed": completed,
                "subtasks": self.temp_subtasks,  # 包含临时创建的子任务
            }
            
            return task_data
            
        except Exception as e:
            logger.error(f"获取任务数据时出错: {e}")
            return None
            
    def save_task(self):
        """保存任务
        
        验证并保存任务数据，如果验证失败则显示错误信息
        """
        try:
            # 获取任务数据
            task_data = self.get_task_data()
            if not task_data:
                raise ValueError("无法获取任务数据")
            ...
            if self.task_data:
                ...
                success = self.scheduler_manager.update_task(task_data)
            else:
                # 创建新任务
                success = self.scheduler_manager.add_task(task_data)
